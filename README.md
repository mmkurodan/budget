...
| 取り込みプレビューを3列マトリクスで編集 | 日付／費目／金額を列ごとに 更新・上に挿入（下が繰り下がる）・削除（下が繰り上がる）。3列の行数が揃った時だけ登録可（`ImportPreviewActivity` + `OcrRecordParser.parseColumns`） |
...
- **対象アプリを指定して（単発）**: 右下の**対象アプリ取得**ボタン → 全画面か**単一アプリ**を選択（Android 14+）
  → 1枚取得 → **3列マトリクス**で列を揃えて（⋮で挿入／削除、セルは直接編集）→「登録」。
